...
def replace_mem(old_i):
    ...
    numreg = 1
    for arg in old_args:
        if isinstance(arg, Temporary) and not isinstance(arg,Register):
            arg = arg.get_alloced_loc()
            after = after + [Instru3A("sd",S[numreg] ,arg)]
            before = before + [Instru3A("ld",S[numreg], arg)]
        ...
        if isinstance(arg, Temporary) :
            ...
            if isinstance(arg,Register) :
                ...
                if not old_i.is_read_only() :
                    if i==1 :
                        after = after + [Instru3A("sd",S[indice] ,arg)]
                        i=0;
                    else :
                        before = before + [Instru3A("ld",S[indice], arg)]
                else :
                    before = before + [Instru3A("ld",S[indice], arg)]
                args.append(S[indice])
                indice+=1
        else :
            args.append(arg)
...
class SmartAllocator(Allocator):
    ...
    def set_gen_kill(self):
        ...
        for ins in self._f.get_instructions():
            ...
            for arg in ins.args:
                ...
                if isinstance(arg, Temporary) or isinstance(arg,Register):
                    if not ins.is_read_only() :
                        if i == 0 :
                            ins._kill.add(arg)
                            i=1;
                        else :
    ...
    def smart_alloc(self, outputname):
        ...
        alloc_dict = {}
        coloring, valeur, templist = self._igraph.color()
        maxcol = len(GP_REGS)-3
        j=0
        for (key,value) in coloring.items() :
            for f in self._f._pool._all_temps :
                if key==str(f) :
                    if value<maxcol :
                        alloc_dict[f]=GP_REGS[value]
                    else :
                        alloc_dict[f] = Offset(FP,-8*(value-maxcol+1))
                        if j>-8*(value-maxcol+1) :
                            j=-8*(value-maxcol+1)
        self._f._stacksize = j
        ...
        if self._debug_graphs:
            print("printing the conflict graph")
            self._igraph.print_dot(outputname,coloring)
